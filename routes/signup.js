...
router.post("/verify",async (req,res,next) =>{
  try {
    ...
    const registerCredientials = await CheckUser(email);
    if(registerCredientials === false){
      await InsertVerifyUser(name,email,password);
      res.status(200).send(true);

    } else if (registerCredientials === true){
        res.status(200).send(false)

    } else  if (registerCredientials === "Server Busy"){
